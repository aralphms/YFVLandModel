...
[
  ...
  spread-speed-to-max-dist                    ; the spread speed calculated to the maximum distance from the centroid of initiator nodes
  list-spread-speed-to-max-dist               ; for model calibration test -> list to store spread-speed-to-max-dist
  list-prop-infected-nodes                    ; for model calibration test -> list to store proportion of infected nodes
  centroid-initiator                          ; the calculated centroid patch for area containing the initiator nodes
...
[
  ...
  initiator?                  ; if the node initiates the virus circulation in the landscape
...
[
  ...
  patch-init?                 ; if the patch pertain to a initiator node
]
...
to setup
  ...
  ask patches [set patch-type "non-forest" set patch-init? false set pcolor white]
  ...
  (ifelse landscape = "1"
    ...
    landscape = "4" [set fragment-dataset gis:load-dataset "Layers_model/LandscapeSP4.shp" set grid-dataset gis:load-dataset "Layers_model/GridLandSP4.shp"]
    landscape = "5" [set fragment-dataset gis:load-dataset "Layers_model/LandscapeSP5.shp" set grid-dataset gis:load-dataset "Layers_model/GridLandSP5.shp"]
    landscape = "6" [set fragment-dataset gis:load-dataset "Layers_model/LandscapeSP6.shp" set grid-dataset gis:load-dataset "Layers_model/GridLandSP6.shp"])
...
; gis:draw fragment-dataset 0.5
  ...
  set mosquito-trans-competence 0.2      ; the assumed probability that an infectious mosquito will transmit the virus to a susceptible host after a successful interaction
  ...
  set total-main-hosts sum [item 0 item 0 host-list] of nodes   ; the total number of main hosts in the landscape
  set YFV-circ 0                 ; the time (in days) the virus is circulating in the landscape

  set list-spread-speed-to-max-dist []   ; for model calibration test -> list to store spread-speed-to-max-dist
  set list-prop-infected-nodes []        ; for model calibration test -> list to store the proportions of infected nodes
...
;   calculating the minimum distance between two linked areas
  ...
  [
    ...
    ask end1
    [
      ; end1 detect the patches of end2 and create an agentset called ref-patches
      ; the constant '1' is added to avoid error messages in rare sutuations where the maximum-distance-link radius fall at the limits of the end2 area
      set ref-patches patches with [node-id = id2] in-radius (maximum-distance-link + 1)
...
;    set viral-introduction viral-introduction + 1 ask my-patches [set pcolor red]]
...
to go
  ...
  calculate-speed                           ;calculates the spreading speed of the virus
...
to insert-the-virus
  ...
  [
    ...
    [
      let init-1 0 let init-2 0 let init-3 0 let init-4 0 let init-5 0
;      set init-1 one-of nodes with [external-node? and ycor < (max-pycor) and ycor > (max-pycor - external-area) ]
      set init-1 one-of nodes with [external-node?]
      ask init-1
      [
        set initiator? true
        set init-2 min-one-of other nodes with [external-node? and initiator? = false][distance myself] ask init-2 [set initiator? true ]
        set init-3 min-one-of other nodes with [external-node? and initiator? = false][distance myself] ask init-3 [set initiator? true ]
        set init-4 min-one-of other nodes with [external-node? and initiator? = false][distance myself] ask init-4 [set initiator? true ]
        set init-5 min-one-of other nodes with [external-node? and initiator? = false][distance myself] ask init-5 [set initiator? true ]
      ]
      ask init-1 [ask my-patches [set patch-init? true]] ask init-2 [ask my-patches [set patch-init? true]] ask init-3 [ask my-patches [set patch-init? true]]
      ask init-4 [ask my-patches [set patch-init? true]] ask init-5 [ask my-patches [set patch-init? true]]
      ask nodes with [initiator?]
...
to calculate-speed
  if ticks > round(start-YFV-circulation) and ticks <= round(start-YFV-circulation + 7) ; The speed will be calculated up to the 7th day after viral emergence
  [
    let number-of-days ticks - start-YFV-circulation
    let patches-init patches with [patch-init?] ; creating an agentset containing all patches pertaining to initiator nodes
    let px-centr mean [pxcor] of patches-init ; calculating the centroid x
    let py-centr mean [pycor] of patches-init ; calculating the centroid y
    set centroid-initiator patch px-centr py-centr ; defining the centroid patch
    ifelse any? nodes with [viral-introduction > 0 and initiator? = false] ; If the virus spread to a non initiator node
   [
      ask centroid-initiator
      [
        ;obtaining the farthest node to the centroid-initiator
        let max-dist-node max-one-of nodes with [viral-introduction > 0 and initiator? = false] [distance myself]
 ;       let px pxcor
 ;       let py pycor
        ; calculating the average daily speed
        set spread-speed-to-max-dist [distancexy px-centr py-centr] of max-dist-node / number-of-days / 10 ; the value 10 converts the distance in patches to km
        set list-spread-speed-to-max-dist lput (spread-speed-to-max-dist) list-spread-speed-to-max-dist ; including the calculated speed to a list
      ]
    ]
    [
      set spread-speed-to-max-dist 0 ; If the virus does not spread to a non initiator node, the distance is 0
      set list-spread-speed-to-max-dist lput (spread-speed-to-max-dist) list-spread-speed-to-max-dist
    ]
    set list-prop-infected-nodes lput prop-infected-nodes list-prop-infected-nodes ; incuding the calculated proportion of infected nodes to a list
  ]
end
...
to-report dead-monkeys-prop
  let num-dead sum [dead-monkeys] of nodes ; summing the number of dead monkeys from all internal nodes
  let prop-dead 0
...
<experiments>
  <experiment name="experiment" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>spread-speed-to-max-dist</metric>
    <metric>prop-infected-nodes</metric>
    <metric>list-spread-speed-to-max-dist</metric>
    <metric>list-prop-infected-nodes</metric>
    <metric>YFV-persistence-days</metric>
    <metric>dead-monkeys-prop</metric>
    <enumeratedValueSet variable="initial-mosquitoes-per-ha">
      <value value="250"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="YFV-initial-burden">
      <value value="0.02"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="maximum-size-to-merge">
      <value value="20"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="external-area">
      <value value="10"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="max-prop-mosq-dispersal">
      <value value="0.1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="maximum-distance-link">
      <value value="10.5"/>
    </enumeratedValueSet>
    ...
    <enumeratedValueSet variable="main-host-per-ha">
      <value value="0.5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="host-move-proportion">
      <value value="0.3"/>
    </enumeratedValueSet>
    ...
    <enumeratedValueSet variable="dead-end-host-per-ha">
      <value value="0.75"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="seed?">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="alternative-host-per-ha">
      <value value="0.5"/>
    </enumeratedValueSet>
